<template>
  <div id="recommend">
    <ul class="infobox" v-for="(item, index) in allDynamic" :key="item.id">
      <li class="list">
        <img class="avatar" :src="item.dynamicimg" alt="" />
        <span class="username">{{ item.username }}</span>
        <div class="right">
          <div
            v-if="allRemarkList.length !== 0"
            @click="isFocus(item.id, index)"
            class="focus"
            ref="focu"
          >
            +关注
      ...
      <li
        class="remarkList"
        v-if="item.id === remark.dynamic_id"
        v-for="(remark,index) in allRemarkList"
        :key="remark.id"
      >
        <img :src="remark.remark_userimg" alt="" />
      ...
      <li class="bottomTools">
        ...
        <van-icon name="chat-o" class="icon" /><span>{{
          item.id === (allRemarkList[index] ? allRemarkList[index].dynamic_id : 0)
...
import { findAllDynamic, dynamic_remark, findAllRemark } from "../../../../api/getData";
export default {
  async mounted() {
    let result = await findAllDynamic();
    console.log(result);
    if (result.data.code === 0) {
      this.allDynamic = result.data.data;
    }
  ...
  methods: {
    ...
    async userRemark(dynamic_id) {
      ...
      if (result.data.code === 0) {
        ...
        this.remark_content = "";
        // console.log(result);
        this.allRemarkList = result.data.data;
      }
      // 当我评论其中一个动态的时候，记录就会显示出来
      let result2 = await findAllRemark(dynamic_id);
    //  let result2 = await findAllRemark(this.allDynamic[0].id);
      console.log(result2);
      if (result2.data.code === 0) {
        // console.log(result2.data);
        this.allRemarkList = result2.data.data;
      }
    },
  },
};
